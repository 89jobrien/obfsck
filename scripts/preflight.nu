...
let has_anthropic = ($env | get -o ANTHROPIC_API_KEY | is-not-empty)
let has_openai = ($env | get -o OPENAI_API_KEY | is-not-empty)
...
let results = [
    ...
    (check "OPENAI_API_KEY set" $has_openai "optional — only needed if using OpenAI-backed features"),
